#!/usr/bin/env bash
set -euo pipefail

BASE="${M5_BASE:-http://127.0.0.1:8100}"
TUNNEL_ID="${M5_TUNNEL_ID:-cf-serena-existing}"
TOKEN="${MCP_STUDIO_GATEWAY_TOKEN:-}"
TMP="$(mktemp -d)"
CERT_WORKER=""
CERT_WORKSPACE="m5.1://external-connectivity-cert-$(date +%s)-$$"

cleanup() {
  if [[ -n "$CERT_WORKER" ]]; then
    curl -sS -X POST "$BASE/api/workers/$CERT_WORKER/release" >/dev/null 2>&1 || true
  fi
  rm -rf "$TMP"
}
trap cleanup EXIT

command -v jq >/dev/null || { echo "FAIL: jq is required"; exit 1; }
[[ -n "$TOKEN" ]] || { echo "FAIL: MCP_STUDIO_GATEWAY_TOKEN is not set in this shell"; exit 1; }

STATUS="$(curl -fsS "$BASE/api/status")"
echo "=== Studio safety state ==="
echo "$STATUS" | jq '.studio | {version,gateway_enabled,gateway_allowed_servers,connectivity_auto_reconnect}'
[[ "$(echo "$STATUS" | jq -r '.studio.gateway_enabled')" == "true" ]] || { echo "FAIL: studio.gateway_enabled must be true"; exit 1; }

ITEM="$(curl -fsS "$BASE/api/connectivity" | jq -c --arg id "$TUNNEL_ID" '.tunnels[] | select(.id==$id)')"
[[ -n "$ITEM" ]] || { echo "FAIL: tunnel $TUNNEL_ID not registered"; exit 1; }
echo "=== Existing Cloudflare tunnel inventory ==="
echo "$ITEM" | jq '{id,provider,name,endpoint,origin,managed,status,error,metadata}'
[[ "$(echo "$ITEM" | jq -r '.provider')" == "cloudflare" ]] || { echo "FAIL: provider is not cloudflare"; exit 1; }
[[ "$(echo "$ITEM" | jq -r '.managed')" == "false" ]] || { echo "FAIL: existing shared tunnel must be registered managed=false"; exit 1; }
[[ "$(echo "$ITEM" | jq -r '.metadata.management_mode // empty')" == "external_remote" ]] || { echo "FAIL: management_mode must be external_remote"; exit 1; }
ENDPOINT="$(echo "$ITEM" | jq -r '.endpoint')"
[[ "$ENDPOINT" == https://* ]] || { echo "FAIL: public endpoint must be https://"; exit 1; }
ROOT="${ENDPOINT%%/mcp/*}/"

# Connectivity plane must never own/kill the shared cloudflared process.
for ACTION in start stop restart; do
  CODE="$(curl -sS -o "$TMP/$ACTION.body" -w '%{http_code}' -X POST "$BASE/api/connectivity/tunnels/$TUNNEL_ID/$ACTION")"
  if [[ "$CODE" != "409" ]]; then
    echo "FAIL: external tunnel $ACTION must be rejected with 409 (got $CODE)"
    cat "$TMP/$ACTION.body"
    exit 1
  fi
done

echo "EXTERNAL_OWNERSHIP_GUARD_PASS"

CERT_WORKER="$(curl -fsS "$BASE/api/workers" | jq -r '.workers[] | select(.state=="idle") | .id' | head -n1)"
[[ -n "$CERT_WORKER" ]] || { echo "FAIL: no idle worker available for isolation certification"; exit 1; }
curl -fsS -X POST "$BASE/api/workers/$CERT_WORKER/bind" \
  -H 'Content-Type: application/json' \
  -d "{\"workspace\":\"$CERT_WORKSPACE\",\"lease_mode\":\"write\",\"metadata\":{\"certification\":\"M5.1-existing-connectivity\"}}" >/dev/null
curl -fsS -X POST "$BASE/api/workers/$CERT_WORKER/state" \
  -H 'Content-Type: application/json' \
  -d '{"state":"busy","work_label":"M5.1 existing tunnel isolation"}' >/dev/null

check_worker() {
  local data state workspace holder
  data="$(curl -fsS "$BASE/api/workers")"
  state="$(echo "$data" | jq -r --arg id "$CERT_WORKER" '.workers[] | select(.id==$id) | .state')"
  workspace="$(echo "$data" | jq -r --arg id "$CERT_WORKER" '.workers[] | select(.id==$id) | .workspace')"
  holder="$(echo "$data" | jq -r --arg ws "$CERT_WORKSPACE" '.leases[] | select(.workspace==$ws) | .worker_id')"
  [[ "$state" == "busy" && "$workspace" == "$CERT_WORKSPACE" && "$holder" == "$CERT_WORKER" ]]
}
check_worker || { echo "FAIL: could not establish synthetic worker/lease"; exit 1; }

INIT='{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-06-18","capabilities":{},"clientInfo":{"name":"mcp-studio-m5.1-external-cert","version":"0.6.2"}}}'

echo "=== Public surface must not expose Studio UI ==="
ROOT_CODE="$(curl -sS -o "$TMP/root.body" -w '%{http_code}' "$ROOT" || true)"
echo "root HTTP=$ROOT_CODE"
if [[ "$ROOT_CODE" == "200" ]] && grep -qi 'MCP Studio' "$TMP/root.body"; then
  echo "FAIL: Studio UI is publicly exposed"
  exit 1
fi

echo "=== Public MCP unauthenticated must fail closed ==="
UNAUTH_CODE="$(curl -sS -o "$TMP/unauth.body" -w '%{http_code}' \
  -X POST "$ENDPOINT" \
  -H 'Accept: application/json, text/event-stream' \
  -H 'Content-Type: application/json' \
  --data "$INIT" || true)"
echo "unauth HTTP=$UNAUTH_CODE"
[[ "$UNAUTH_CODE" == "401" ]] || { cat "$TMP/unauth.body"; echo "FAIL: expected HTTP 401 without bearer token"; exit 1; }

remote_probe() {
  : > "$TMP/headers"
  local code sid notify_code tools_code delete_code
  code="$(curl -sS -D "$TMP/headers" -o "$TMP/init.body" -w '%{http_code}' \
    -X POST "$ENDPOINT" \
    -H "Authorization: Bearer $TOKEN" \
    -H 'Accept: application/json, text/event-stream' \
    -H 'Content-Type: application/json' \
    --data "$INIT")"
  [[ "$code" == "200" ]] || { echo "authenticated initialize HTTP=$code"; cat "$TMP/init.body"; return 1; }
  grep -Eq '"serverInfo"|"protocolVersion"' "$TMP/init.body" || { cat "$TMP/init.body"; return 1; }
  sid="$(awk 'BEGIN{IGNORECASE=1} /^mcp-session-id:/ {gsub("\\r", "", $2); print $2; exit}' "$TMP/headers")"
  [[ -n "$sid" ]] || { echo "missing Mcp-Session-Id"; cat "$TMP/headers"; return 1; }
  notify_code="$(curl -sS -o "$TMP/notify.body" -w '%{http_code}' -X POST "$ENDPOINT" \
    -H "Authorization: Bearer $TOKEN" -H "Mcp-Session-Id: $sid" \
    -H 'Accept: application/json, text/event-stream' -H 'Content-Type: application/json' \
    --data '{"jsonrpc":"2.0","method":"notifications/initialized"}')"
  case "$notify_code" in 200|202|204) ;; *) echo "initialized notification HTTP=$notify_code"; return 1;; esac
  tools_code="$(curl -sS -o "$TMP/tools.body" -w '%{http_code}' -X POST "$ENDPOINT" \
    -H "Authorization: Bearer $TOKEN" -H "Mcp-Session-Id: $sid" \
    -H 'Accept: application/json, text/event-stream' -H 'Content-Type: application/json' \
    --data '{"jsonrpc":"2.0","id":2,"method":"tools/list"}')"
  [[ "$tools_code" == "200" ]] || { echo "tools/list HTTP=$tools_code"; cat "$TMP/tools.body"; return 1; }
  grep -q 'herdr_' "$TMP/tools.body" || { echo "tools/list missing Herdr tools"; cat "$TMP/tools.body"; return 1; }
  delete_code="$(curl -sS -o /dev/null -w '%{http_code}' -X DELETE "$ENDPOINT" \
    -H "Authorization: Bearer $TOKEN" -H "Mcp-Session-Id: $sid" \
    -H 'Accept: application/json, text/event-stream')"
  case "$delete_code" in 200|202|204|404|405) ;; *) echo "session DELETE HTTP=$delete_code"; return 1;; esac
  echo "REMOTE_MCP_PROBE_PASS session=$sid"
}

echo "=== Authenticated MCP round-trip through existing tunnel ==="
remote_probe
check_worker || { echo "FAIL: public MCP probe mutated execution state"; exit 1; }

# Exercise repeated connectivity observations. This is intentionally non-destructive:
# the systemd/Cloudflare-owned connector is never signalled or restarted by Studio.
echo "=== Non-destructive connectivity observation ==="
for _ in 1 2 3; do
  SNAP="$(curl -fsS -X POST "$BASE/api/connectivity/run")"
  TITEM="$(echo "$SNAP" | jq -c --arg id "$TUNNEL_ID" '.tunnels[] | select(.id==$id)')"
  echo "$TITEM" | jq '{id,status,error,last_healthy_at,managed,metadata}'
  [[ "$(echo "$TITEM" | jq -r '.status')" == "healthy" ]] || { echo "FAIL: external tunnel probe is not healthy"; exit 1; }
  check_worker || { echo "FAIL: connectivity observation mutated worker/lease"; exit 1; }
  sleep 1
done

curl -fsS -X POST "$BASE/api/workers/$CERT_WORKER/release" | jq '{id,state,workspace,lease_mode}'
CERT_WORKER=""

echo "M5_1_EXISTING_CLOUDFLARE_LIVE_CERTIFICATION_PASS"
